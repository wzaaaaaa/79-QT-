...
//  连接到节拍器beat
    QPushButton *button = new QPushButton("beat", this);
    button->setStyleSheet("QPushButton {"
                          "font: 87 9pt 'Arial Black';" // 保持字体风格和大小一致
                          "font-size: 20px;"           // 字体大小
                          "border-radius: 25px;"       // 增加半径使角更圆
                          "background-color: #F5F5DC;"     // 背景颜色
                          "border: none;"              // 无边框
                          "color: black;"              // 设置文字颜色为白色
                          "}");

    button->setGeometry(60, 72, 70, 70);
...
//  连接到曲库
    QPushButton *library = new QPushButton(this);
    library->move(895, 380);  // 将按钮移动
    library->setFixedSize(80, 80);  // 设置按钮的大小
    library->setStyleSheet("QPushButton {"
                          "border-radius: 25px;"  // 设置边框半径为50像素，使按钮呈现为圆形
                          "background-color: white;"  // 设置背景颜色
                          "}");
    QIcon buttonIcon("path");  // 使用图片文件路径替换
    library->setIcon(buttonIcon);
    library->setIconSize(QSize(50, 50));  // 设置图标的大小


    connect(library, &QPushButton::clicked, this, &Piano::viewSheetMusic);

}


void Piano::viewSheetMusic() {
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open Sheet Music"), "", tr("Image Files (*.png *.jpg *.jpeg)"));
    if (!fileName.isEmpty()) {
        QLabel *imageLabel = new QLabel;
        QPixmap pixmap(fileName);

        if (!pixmap.isNull()) {
            imageLabel->setPixmap(pixmap.scaled(800, 600, Qt::KeepAspectRatio, Qt::SmoothTransformation));  // 调整大小并保持纵横比
            imageLabel->setWindowTitle(tr("Sheet Music"));
